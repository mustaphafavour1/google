...
import type { WhyMeContent } from "@/lib/types";

/** Cycles by card position — icons aren't CMS-editable, only the copy is. */
const ICONS = [Gem, Palette, Layers3, ShieldCheck];
...
export function WhyMeSection({ content }: { content: WhyMeContent }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary-500 to-primary-600 px-4 py-16 text-white sm:px-8">
      <div className="relative mx-auto max-w-2xl text-center">
        <p className="type-eyebrow mb-2.5 text-white/70">{content.eyebrow}</p>
        <h2 className="text-[clamp(1.875rem,4vw,2.75rem)] font-bold leading-tight tracking-tight text-white">
          {content.title}
        </h2>
        <p className="mt-3.5 text-[17px] font-medium leading-relaxed text-white/80">{content.subtitle}</p>
      </div>
