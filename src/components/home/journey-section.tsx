  ...
  return (
    <div>
      <div className="relative mx-auto aspect-[1000/430] w-full max-w-4xl">
        <span className="absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-90 whitespace-nowrap text-[10px] font-medium uppercase tracking-wide text-ink-faint">
          Design impact
        </span>
          ...
          return (
            <div key={`${milestone.year}-${i}`} className="absolute" style={{ left: `${xPercent}%`, top: `${yPercent}%` }}>
              ...
              <motion.div
                ...
                className="absolute w-[6.5rem] max-w-[9rem] -translate-x-1/2 rounded-md border border-hairline bg-surface px-1.5 py-1 text-[8px] leading-snug text-ink-soft shadow-[0_4px_10px_rgb(35_25_15_/_0.08)] sm:w-28"
                style={{
                  top: above ? "-4.75rem" : "0.75rem",
                  transform: `translateX(-50%) rotate(${TILTS[i % TILTS.length]}deg)`,
                }}
              >
                <span className="data-mono block text-[8px] font-semibold text-primary-500">{label}</span>
                {milestone.text}
              </motion.div>
            </div>
          );
        ...
        <div className="absolute inset-x-0 bottom-[-1.6rem] flex justify-between px-1">
          {milestones.map((milestone, i) => (
            <span key={`axis-${milestone.year}-${i}`} className="type-meta text-[10px]">
