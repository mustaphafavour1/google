...
const FORM_NAME_ENTRY_ID = process.env.NEXT_PUBLIC_COMMENT_FORM_NAME_ENTRY_ID;
      ...
      <form action={FORM_URL} method="POST" target={FRAME_NAME} onSubmit={handleSubmit} className="flex flex-col gap-2">
        {FORM_NAME_ENTRY_ID && (
          <input
            ...
            placeholder={configured ? "Your name (optional)" : "Comments aren't set up yet"}
            disabled={!configured}
            className="h-10 rounded-md border border-border bg-transparent px-3 text-[13px] text-ink-strong placeholder:text-ink-muted disabled:cursor-not-allowed disabled:opacity-60"
          />
        )}
